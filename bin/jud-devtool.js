...
function startServerAndLaunchDevtool(entry) {
    ...
    if (entry) {
        Config.entryBundleUrl = 'http://' + ip + ':' + port + Path.join('/' + Config.bundleDir, Path.basename(entry).replace(/\.(ju|vue)$/, '.js')).replace(/\\/g, '/');
    ...
    if (Config.entryBundleUrl) {
        ...
        if (!/wh_jud=true/.test(Config.entryBundleUrl) && !urlObj.query['_jud_tpl']) {
            urlObj.query['_jud_tpl'] = Config.entryBundleUrl;
            urlObj.search = '';
