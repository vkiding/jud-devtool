...
wsRouter.all('/debugProxy/debugger/:sessionId', function*(next) {
    ...
    if (!P2PSession.join(this.params.sessionId, this.websocket)) {
        //P2PSession.postMessage(this.websocket, {method: "JudDebug.reload"});
    }
    else {
        //console.log('enable remote debug');
        //P2PSession.postMessage(this.websocket, {method: "JudDebug.enable"});
    }
    this.websocket.on('message', function (message) {
        message = JSON.parse(message);
        Logger.printMessage(message, 'chrome');
        if (message.method === 'JudDebug.enable' || message.method === 'JudDebug.disable') {
            var peer = P2PSession.findOppositePeer(this);
            if (peer) {
                let device = DeviceManager.getDeviceById(peer.websocket._deviceId);
                if (device) {
                    device.deviceInfo.remoteDebug = message.method === 'JudDebug.enable' ? true : false;
                }
            }
        }
        P2PSession.postMessage(this, message);
...
DeviceManager.on('update', function (deviceList) {
    listPageWebsocket.forEach(ws=> {
        if (ws.readyState == 1) {
            ws.send(JSON.stringify({method: "JudDebug.pushDeviceList", params: deviceList}));
        }
    })
});
...
MessageBus.on('page.refresh', function () {
    DeviceManager.getDeviceList().forEach(function (device) {
        ...
        if (device.deviceInfo.platform.toLowerCase() == 'android' && device.deviceInfo.devtoolVersion >= '0.0.8.5' || device.deviceInfo.platform.toLowerCase() == 'ios' && device.deviceInfo.devtoolVersion >= '0.8.0') {
            devicePeer.send({method: 'JudDebug.refresh'})
        }
        else {
            devicePeer.send({method: 'JudDebug.reload'})
        }
        //device.debuggerSession.postMessage(device.websocket, {method: 'JudDebug.refresh'})
    });
    /* listPageWebsocket.forEach(ws=> {
     ws.send(JSON.stringify({method: "JudDebug.refreshPage"}));
     })*/
});
wsRouter.all('/debugProxy/list', function*(next) {
    ...
    this.websocket.on('message', function (messageText) {
        ...
        if (message.method == 'JudDebug.setLogLevel') {
            ...
            if (device && device.websocket.readyState == 1) {
                ...
                let targetMsg = {method: 'JudDebug.setLogLevel', params: {logLevel: message.params.data}};
        ...
        else if (message.method == 'JudDebug.setRemoteDebug') {
            let device = DeviceManager.getDeviceById(message.params.deviceId);
            if (device && device.websocket.readyState == 1) {
                device.deviceInfo.remoteDebug = message.params.data;
                device.websocket.send(JSON.stringify({method: 'JudDebug.' + (message.params.data ? 'enable' : 'disable')}));
            }
        }
        else if (message.method == 'JudDebug.setElementMode') {
            let device = DeviceManager.getDeviceById(message.params.deviceId);
            if (device && device.websocket.readyState == 1) {
                device.deviceInfo.elementMode = message.params.data;
                device.websocket.send(JSON.stringify({
                    method: 'JudDebug.setElementMode',
                    params: {mode: message.params.data}
                }));
            }
        }
        else if (message.method == 'JudDebug.refreshDevice') {
            let device = DeviceManager.getDeviceById(message.params.deviceId);
            if (device && device.websocket.readyState == 1) {
                device.websocket.send(JSON.stringify({method: 'JudDebug.reload'}));
            }
        }
        else if (message.method == 'JudDebug.network') {
            let device = DeviceManager.getDeviceById(message.params.deviceId);
            if (device && device.websocket.readyState == 1) {
                console.log(message);
                device.deviceInfo.network = message.params.enable;
                device.websocket.send(JSON.stringify({
                    method: 'JudDebug.network', params: {
...
wsRouter.all('/debugProxy/native', function*(next) {
    ...
    this.websocket.on('message', function (messageText) {
        ...
        if (message.method) {
            ...
            if (device) {
                if (message.result && message.result.method === 'JudDebug.syncReturn') {
