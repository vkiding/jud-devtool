...
function connect() {
    ...
    websocket.onmessage = function (event) {
        ...
        if (message.method == 'JudDebug.pushDeviceList') {
            renderDeviceList(message.params);
        }
        else if (message.method == 'JudDebug.setEntry') {
            message.params.forEach(function (bundleUrl, i) {
                var urls = bundleUrl.split('_jud_tpl='), url, name;
                if (urls.length >= 2) {
        ...
        else if (message.method == 'JudDebug.refreshPage') {
    ...
    var html = deviceList.map(function (device) {
        return `
            <div class="device-wrap" id="device_${device.deviceId}">
                ...
                <div class="line"><span>JudVersion</span><b>${device.judVersion}</b></div>
    ...
    if(html.length>0) {
        ...
        inspectModeList.forEach(function(inspectModeSelector){
            ...
            swt.onchange=function(evt){
                ...
                websocket.send(JSON.stringify({method:'JudDebug.setRemoteDebug',params:{
                    deviceId:deviceId,
                    data:el.checked
                }}));
                if(!el.checked){
        ...
        document.querySelectorAll('.switch-network').forEach(function(swt){
            swt.onchange=function(evt){
                var el=evt.target;
                var deviceId=el.getAttribute('x-data');
                websocket.send(JSON.stringify({method:'JudDebug.network',params:{
                    deviceId:deviceId,
                    enable:el.checked
                }}));
            }
...
function openDebugger(deviceId) {
    ...
    var $JudInspectorProxy=`ws://${location.host + '/debugProxy/inspector/' + device.inspectorSessionId}`;
    device.debuggerWindow.$JudInspectorProxy=$JudInspectorProxy;
    device.debuggerWindow.device=device;
    device.debuggerWindow.sessionStorage.setItem('$JudInspectorProxy',$JudInspectorProxy);
...
var switchComponent=function(device,label){
    ...
    connect();
createQRCode('switchQrcode', `http:\/\/${location.host}/devtool_fake.html?_jud_devtool=ws:\/\/${location.host}/debugProxy/native`);
